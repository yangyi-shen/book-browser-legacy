  ...
  if (bookList.bookDepo) {
    return (
      <div className='App'>
        ...
        <main>
          {bookList.allBooks.map((book, index) => <BookItem key={index} image={book.image} title={book.title} price={book.price} format={book.format} author={book.author} bookstore={book.bookstore} />)}
        </main>
      </div>
    )
  } else {
